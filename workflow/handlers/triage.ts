...
import {askJson, type LlmOptions} from '../lib/llm'
import {FLAG_ALLOWLIST, route, type Route} from '../lib/routing'

export type Llm = Omit<LlmOptions, 'content'>

interface CaseForTriage {
  _id: string
  caseNumber?: string
  title?: string
  category?: string
  ...
  objectShape?: string
  evidence?: {_type: string; caption?: string; quote?: string; description?: string; lengthCm?: number}[]
  witness?: {codename?: string; credibility?: number; reportsCount?: number; closedCounts?: Record<string, number>}
  triage?: {plausibility?: number; flags?: string[]; effectKey?: string; model?: string; route?: Route}
}
...
const INSTRUCTION = `You are the Field Investigator at the Cryptid Field Office, a deadpan government bureau that records reports of unexplained sightings: cryptids, UFOs, extraterrestrial encounters, ghosts, spirits and jinn, and occult activity.
...
function validate(raw: unknown, slugs: Set<string>): TriageOut {
  ...
  const flags = Array.isArray(r.flags) ? r.flags.filter((f): f is string => typeof f === 'string' && (FLAG_ALLOWLIST as readonly string[]).includes(f)) : []
...
const outputsFor = (r: Route) => ({outputs: {needsHuman: r === 'review', likelyHoax: r === 'auto-debunk'}})

export function makeTriageHandler(content: SanityClient, llm: Llm): EffectHandler {
  return async (params, ctx) => {
    const caseId = extractDocumentId(String(params.subject))
    const c = await content.fetch<CaseForTriage | null>(CASE_QUERY, {id: caseId})
    if (!c) {
      ctx.log(`agent-triage: case ${caseId} not found; routing to review`)
      return outputsFor('review')
    }

    // Replay guard: this exact dispatch already wrote its triage.
    if (c.triage?.effectKey === ctx.effectKey && c.triage.route) return outputsFor(c.triage.route)

    // Seed guard: seeded cases carry a stored triage, so seeding spends no AI quota.
    ...
    const fallback = async (reason: string): Promise<ReturnType<typeof outputsFor>> => {
      await content
        .patch(caseId)
        .set({triage: {summary: 'The Field Investigator was unavailable. Referred to the Director.', model: 'none', route: 'review', triagedAt: new Date().toISOString(), effectKey: ctx.effectKey, flags: []}})
        .commit()
      await appendLog(content, caseId, {key: `${ctx.effectKey}:triaged`, actor: 'system', kind: 'error', message: `${reason} Referred to the Director.`})
      return outputsFor('review')
    }

    // Kill switch and daily budget.
    const settings = await content.fetch<{agentEnabled?: boolean; dailyAgentBudget?: number} | null>(
      `*[_id == "bureau-settings"][0]{agentEnabled, dailyAgentBudget}`,
    )
    ...
    const used = await content.fetch<number>(
      `count(*[_type == "case" && defined(triage.triagedAt) && triage.model != "none" && triage.model != "seed" && dateTime(triage.triagedAt) > dateTime(now()) - 86400])`,
    )
    if (used >= budget) return fallback('The daily triage budget has been reached.')

    const subjects = await content.fetch<{_id: string; slug: string; name: string; category: string; signatureTraits?: string[]}[]>(
      `*[_type == "subject"]{_id, "slug": slug.current, name, category, signatureTraits}`,
    )
    ...
    try {
      result = await askJson(
        ...
        INSTRUCTION,
        {
          report: JSON.stringify({caseNumber: c.caseNumber, title: c.title, category: c.category, ...c, witness: undefined, triage: undefined, source: undefined, hidden: undefined}),
          subjects: JSON.stringify(subjects.map(({slug, name, category, signatureTraits}) => ({slug, name, category, signatureTraits}))),
    ...
    if (c.category === 'spirit') flags.add('faith-sensitive') // deterministic: never left to the model
    ...
    await content
      ...
      .set({
        triage: {
          ...
          ...(match ? {subjectMatch: {_type: 'reference', _ref: match._id}} : {}),
        },
...
function routeSentence(r: Route): string {
